...
fs.readFile('./config.json', function (err, data) {
	...
	socketManagerUDPPort = JSON.parse(data).socketManagerUDPPort;
	rootPath = JSON.parse(data).rootPath;


	//UDP通信端口绑定,用于指定UDP监听message端口		
	s.bind(localUDPPort,"127.0.0.1");
	
	console.log("UDP datagrame bind to [127.0.0.1] on port ["+localUDPPort+"].");			
							...
							console.log(err);
								...
								archive.addFiles([{name:fileName,path:file1},{name:"small_"+fileName,path:file2}],function(err){
									...
									fs.writeFile("./upload/"+appid+'/'+fileNameUncap+'.zip', buff, function(){
										...
										s.send(buf, 0, buf.length, socketManagerUDPPort, "127.0.0.1", function(err) {
											if(err){
